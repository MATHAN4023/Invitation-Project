    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 p-4 bg-gray-100">
        {selectedTemplate ? (
          ...
          >
            <h1 className="text-2xl font-bold mb-2">Editing: {selectedTemplate.name}</h1>
            <img 
            ...
            {textElements.map((text, index) => (
              <div key={index} className="text-xl font-bold mb-2">{text}</div>
            ))}
          </div>
        ) : (
          <h1 className="text-2xl font-bold">Select a template to start editing!</h1>
        )}
      </div>
    </div>
  );
